...
function getWorkspacesYarn(cwd) {
	...
	let packageWorkspaces = manifest.workspaces;
	if (packageWorkspaces && packageWorkspaces.packages) {
		packageWorkspaces = packageWorkspaces.packages;
	}
	// Only continue if manifest.workspaces is an array of strings.
	if (!checker("string[]+")(packageWorkspaces))
	...
	const workspaces = glob.sync(
		packageWorkspaces.map((p) => p.replace(/\/?$/, "/package.json")),
		{
			cwd: cwd,
			realpath: true,
			ignore: "**/node_modules/**",
